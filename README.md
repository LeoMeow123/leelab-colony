...
## Using it
- **Colony** (home): search (tag / strain), sex, genotype, facility, status, age window, owner, location; sort by any column; **individual or grouped** view; click a row for the detail + timeline; **CSV** exports the filtered set. Status defaults to *any* so nothing a member adds is hidden. **Breeding cages** appear in their own purple section **below the colony table** (grouped by cage, `M+F · counts · DOB+DOB`), kept out of the main list and the sex/age grouping and **not requestable**. Here the section is **read-only for members** (managers/PI can still **Wean**); members wean and edit their own cages in **My colony**. Guardian-angel pets watch over the top.
