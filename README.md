...
- **🗺 Map:** a visual room/rack layout. Drill **Rooms → room → rack**; each rack shows its two sides (**Front A–G / Back H–N**, rows 1–9) as a grid of cages colored by **genotype or status**, breeding cells marked 🔬. A **range** like `H–L` (a cohort recorded as filling columns H–L) renders as a **striped block** over that span with a summary chip — not hidden away. Click a cage or block to list its mice. Then **drag to move, with a confirm before any write**: a whole cage onto an empty cell, or open a cage/block and **drag an individual mouse** onto a cell to pin it precisely. Only truly location-less mice sit in an **Unplaceable** tray (also droppable). Moves are owner-scoped + audited. CRAF 57 + 157 are shown as one room; EBS as 4 home cages. Cached, with a ↻ Refresh.
