...
- **🗺 Map:** a visual room/rack layout. Drill **Rooms → room → rack**; each rack shows its two sides (**Front A–G / Back H–N**, rows 1–9) as a grid of cages colored by **genotype or status**, breeding cells marked 🔬. Click a cage to see its mice. **Drag a cage onto an empty cell to move it** (you confirm first; owner-scoped + audited), and **drag a "to place" chip** (mice imported with a whole-rack range, so not yet pinned to a cell) onto a cell to place it. CRAF 57 + 157 are shown as one room; EBS as 4 home cages; location-less mice sit in an **Unplaced** bucket. Read from the DB and cached, with a ↻ Refresh.
