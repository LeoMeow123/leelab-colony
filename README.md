...
memorial for departed mice; a **😇 Guardian Angels** wall of the lab’s pets & loved ones; CSV
...
- **🗺 Map:** a visual room/rack layout. Drill **Rooms → room → rack**; each rack shows its cages as a grid colored by **genotype or status**. Rack geometry is per-rack: CRAF/SAF are two-sided (**Front A–G / Back H–N**, rows 1–9); **EBS 048’s IVSS** rack is single-sided (**A–F, rows 1–10**). **Breeding cages get a distinct fixed color (pink)** whatever the color mode — with a **🔬** marker, their **genotype(s)** written on the cell, and a “🔬 Breeding cage” legend entry — so they're obvious at a glance. A **range** like `H–L` (a cohort recorded as filling columns H–L) renders as a **striped block** over that span, **labeled right on it with genotype · age · sex** (e.g. `Tau · 25.6mo · 9M+9F`) so the basics need no click, plus a matching summary chip below. Click a cage or block to list its mice. Then **drag to move, with a confirm before any write**: a whole cage onto an empty cell, or open a cage/block and **drag an individual mouse** onto a cell to pin it precisely. Only truly location-less mice sit in an **Unplaceable** tray (also droppable). Use **↔ Transfer to another rack** to open a **second rack side-by-side** and **drag mice between racks/rooms** — a cross-rack drop updates the mouse's full location (facility / room / rack / cell). Moves are owner-scoped + audited. CRAF 57 + 157 are shown as one room. **EBS room 58** is shown as its **4 specialized home cages** (Cam01–Cam04) — drag a mouse into a home cage (with confirm) to assign it; **EBS room 048** is the **IVSS** rack grid. Registered racks (like IVSS) appear even before any mice are placed in them. Cached, with a ↻ Refresh.
...
**Strain vs genotype (important, and counterintuitive):** by lab convention **strain** = the
*line* (WT, Tau, APP / APP-NL-F) and is stored in the **`cohort`** column; **genotype** = the
*construct / zygosity* (C57BL/6, hMAP, +/+, +/-) and is stored in the **`genotype_label`** column.
(The column names are historical — despite its name, `genotype_label` holds the genotype and
`cohort` holds the strain.) The Colony filters + table columns and the ⚠ Strain/Geno fix tab all
follow this mapping.
