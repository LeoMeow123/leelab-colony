...
## Using it
- **Colony** (home): search (tag / strain), sex, genotype, facility, status, age window, owner, location; sort by any column; **individual or grouped** view; click a row for the detail + timeline; **CSV** exports the filtered set. Status defaults to *any* so nothing a member adds is hidden. **Breeding cages** appear in their own purple section (grouped by cage, `M+F · counts · DOB+DOB`), kept out of the main list and the sex/age grouping and **not requestable**; **Wean** releases them into the colony. Guardian-angel pets watch over the top.
